...
public interface AvailabilityRepository extends JpaRepository<Availability, Long> {
    ...
    // 특정 튜터의 가능한 시간대 조회
    List<Availability> findByTutorIdAndStartTimeBetweenAndIsAvailableTrue(
            Long tutorId,
            LocalDateTime startTime,
            LocalDateTime endTime
    );
    ...
    List<Availability> findByStartTimeBetweenAndIsAvailableTrue(
            LocalDateTime startTime,
            LocalDateTime endTime
    );

    // 특정 시간대의 가능한 시간대 조회
    List<Availability> findByStartTimeAndEndTimeAndIsAvailableTrue(
            LocalDateTime startTime,
            LocalDateTime endTime
    );
}